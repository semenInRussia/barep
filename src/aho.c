...
bool aho_is_match(Aho_Node_Ptr x) { return aho_heap[x].pattern != -1; }
...
int aho_pattern(Aho_Node_Ptr x) { return aho_heap[x].pattern; }
...
Aho_Node_Ptr aho_make() {
  ...
  ++aho_heap_top;
  aho_heap[j].pattern = aho_heap[j].link = -1;
  aho_heap[j].cnt_matches = 0;
...
void aho_build(Aho_Node_Ptr root) {
  ...
  while (q.count > 0) {
    ...
    for (size_t i = 0; i < AHO_ALPHABET; i++) {
      ...
      t->link = lnk;
      t->size = aho_size(x) + 1;
      t->cnt_matches = aho_matches_count(lnk) + aho_is_match(y);
      t->match_size = aho_is_match(y) ? aho_size(y) : aho_match_size(lnk);
      t->nxt_match = aho_is_match(lnk) ? lnk : aho_next_match(lnk);
      aho_queue_push(&q, y);
    }
  }
}
